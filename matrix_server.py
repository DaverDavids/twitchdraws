...
def run_test_mode(interval_sec, num_pixels):
    ...
    at a specific interval, ignoring global config toggles.
    """
    try:
    ...
    log_print(f"Starting test mode: {num_pixels} pixels every {interval_sec}s")

    while not shutdown_event.is_set():
        try:
            actual_count = min(num_pixels, PIXEL_COUNT)
            indexes = random.sample(range(PIXEL_COUNT), k=actual_count)
            ...
            log_print(f"Test update: {actual_count} pixels refreshed.")
        except Exception as e:
            log_error("Error in independent test thread", exc=e)
...
def parse_pixel_command(message):
    ...
    if not re.match(r"^!pixel\b", msg, re.IGNORECASE):
        return
